...
export function canMove(board: Board, direction: Direction): boolean {
  ...
  const nextBox = board.boxes.find(
    (box) => box.x === player.x + vector.x && box.y === player.y + vector.y
  );
  ...
  if (nextBox) {
    ...
    const nextNextBox = board.boxes.find(
      (box) => box.x === nextBox.x + vector.x && box.y === nextBox.y + vector.y
    );
    if (nextNextBox) {
      return false;
    }
...
export function move(
  board: Board,
  direction: Direction
): { next: Board & { action: Action }; hasPushed: boolean } {
  const player = board.player;
  const vector = getDirectionVector(direction);
  ...
  const newBoxes = board.boxes.map((box) => {
    if (box.x === newPlayer.x && box.y === newPlayer.y) {
      ...
      return {
        ...
        x: newPoint.x,
        y: newPoint.y,
        isOnButton: board.tiles[newPoint.y][newPoint.x] === BoardTile.Button,
      };
    }
    return box;
  });
  ...
  const newBoxes = board.boxes.map((box) => {
    ...
    if (boxAction) {
      ...
      return {
        ...
        y: box.y - boxVector.y,
        isOnButton:
          board.tiles[box.y - boxVector.y][box.x - boxVector.x] ===
          BoardTile.Button,
      };
    }
    return box;
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (tile === LevelTile.Box || tile === LevelTile.ButtonBox) {
        boxes.push({
          ...
          y,
          id: boxes.length,
          isOnButton: tile === LevelTile.ButtonBox,
        });
      }
    }
  }
